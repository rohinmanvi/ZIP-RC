...
Example usage:
    ...
    python train_ziprc_joint_head.py --model-id deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B \
                                     --weights-path models/zip_model --data-path data/data.parquet

    # Custom reward values (e.g., 5-level quality score)
    python train_ziprc_joint_head.py --model-id deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B \
                                     --weights-path models/zip_model --data-path data/data.parquet \
                                     --reward-values 0.0 0.25 0.5 0.75 1.0
"""
...
def train(model, dataset, distribution_token_id, num_bins, weights_path,
    ...
    optimizer = AdamW([p for p in model.parameters() if p.requires_grad], 
                      lr=learning_rate, betas=(beta_1, beta_2), weight_decay=weight_decay)
...
def main_worker(local_rank, world_size, cfg):
    ...
    if cfg.kl_coefficient > 0:
        try:
            ...
            if local_rank == 0:
                print(f"flash_attention_2 not available for ref model ({e}); using default attention.", flush=True)
            ref_model = AutoModelForCausalLM.from_pretrained(
                cfg.model_id,
                torch_dtype=torch.bfloat16,
                trust_remote_code=True,
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=10.0,
        help="Coefficient for KL divergence loss from reference model",
    )
